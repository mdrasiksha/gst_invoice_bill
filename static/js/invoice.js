(function(){
  ...
  const companyStateCode=()=>{const selected=field('company_state')?.selectedOptions?.[0]; return ((selected?.dataset.code||form.dataset.companyStateCode||document.body.dataset.companyState||'').toString().trim().padStart(2,'0'));};
  ...
  function updateText(){document.querySelectorAll('[data-preview]').forEach(el=>{const f=field(el.dataset.preview); if(f)el.textContent=f.value;}); const terms=document.getElementById('previewTerms'); const termsField=field('terms'); if(terms&&termsField)terms.textContent=termsField.value; const companyName=field('company_name'); const previewCompany=document.querySelector('.preview-company'); if(companyName&&previewCompany){const title=previewCompany.querySelector('h2'); if(title) title.textContent=companyName.value||'Your Company'; const state=field('company_state')?.value||''; const details=[field('company_gstin')?.value?`GSTIN: ${field('company_gstin').value}`:'', [field('company_address')?.value,field('company_city')?.value,state,field('company_pincode')?.value].filter(Boolean).join(', '), [field('company_phone')?.value?`Phone: ${field('company_phone').value}`:'',field('company_email')?.value?`Email: ${field('company_email').value}`:''].filter(Boolean).join(' · ')].filter(Boolean); previewCompany.querySelectorAll('p').forEach(p=>p.remove()); details.forEach(text=>{const p=document.createElement('p'); p.textContent=text; previewCompany.appendChild(p);});}}
