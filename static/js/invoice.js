(function(){
  ...
  function customerData(){
    if(isNewCustomer()){
      ...
      return {name:get('new_customer_name'), gstin:get('new_customer_gstin'), address, phone:get('new_customer_phone'), email:get('new_customer_email'), state, stateCode};
    }
    const sel=field('customer_id'); const opt=sel?.options[sel.selectedIndex];
    return {name:opt?.text||'', gstin:opt?.dataset.gstin||'', address:opt?.dataset.address||'', phone:opt?.dataset.phone||'', email:opt?.dataset.email||'', state:opt?.dataset.state||'', stateCode:opt?.dataset.stateCode||''};
  }
  ...
  function updateCustomerMode(){
    ...
    const sel=field('customer_id'); if(sel) sel.required=false;
    updateCustomer();
  }
  function updateText(){document.querySelectorAll('[data-preview]').forEach(el=>{const f=field(el.dataset.preview); if(f)el.textContent=f.value;}); const terms=document.getElementById('previewTerms'); const termsBlock=document.getElementById('previewTermsBlock'); const termsField=field('terms'); if(terms&&termsField){const value=termsField.value.trim()||DEFAULT_TERMS; terms.textContent=value; termsBlock?.classList.remove('d-none');}}
  function recalc(){ if(!table) return; let taxable=0,gst=0,total=0,html=''; table.querySelectorAll('tbody tr').forEach((row,idx)=>{const name=row.querySelector('[name="item_name[]"]').value||''; const hsn=row.querySelector('[name="hsn_sac[]"]').value||''; const qty=parseFloat(row.querySelector('[name="quantity[]"]').value)||0; const rate=parseFloat(row.querySelector('[name="unit_price[]"]').value)||0; const gstRate=parseFloat(row.querySelector('[name="gst_percentage[]"]').value)||0; const tx=Math.max(qty,0)*Math.max(rate,0); const gstAmt=tx*gstRate/100; const line=tx+gstAmt; row.querySelector('.line-total').textContent=money(line); taxable+=tx; gst+=gstAmt; total+=line; html+=`<tr><td>${idx+1}</td><td>${escapeHtml(name)||'-'}</td><td class="text-center">${escapeHtml(hsn)||'-'}</td><td class="text-center">${qty}</td><td class="text-center">${money(rate)}</td><td class="text-center">${gstRate}%</td><td class="text-center">${money(tx)}</td><td class="text-center">${money(gstAmt)}</td><td class="text-center">${money(line)}</td></tr>`; }); const intra=isIntra(); const rounded=Math.round(total); const roundOff=rounded-total; const set=(id,val)=>{const el=document.getElementById(id); if(el) el.textContent=money(val)}; set('taxableTotal',taxable); const maxGst=Math.max(...Array.from(table.querySelectorAll('[name="gst_percentage[]"]')).map(el=>parseFloat(el.value)||0),0); const label=(id,text)=>{const el=document.getElementById(id); if(el)el.textContent=text}; label('cgstLabel',`CGST (${intra?maxGst/2:0}%)`); label('sgstLabel',`SGST (${intra?maxGst/2:0}%)`); label('igstLabel',`IGST (${intra?0:maxGst}%)`); set('cgstTotal',intra?gst/2:0); set('sgstTotal',intra?gst/2:0); set('igstTotal',intra?0:gst); set('roundOffTotal',roundOff); set('grandTotal',rounded); const words=document.getElementById('previewWords'); if(words)words.textContent=toWords(rounded); const pi=document.getElementById('previewItems'); if(pi) pi.innerHTML=html||'<tr><td colspan="9" class="text-muted">Add an item</td></tr>'; updateCustomer(); updateText(); }
